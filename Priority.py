    ...
    for i in range(len(List) - 1):
        for j in range(len(List) - 1 - i):
            if float(List[j]['Arrival_Time']) > float(List[j + 1]['Arrival_Time']):
                ...
                if int(List[j]['Priority']) > int(List[j + 1]['Priority']):
                    tmp = List[j]
                    List[j] = List[j + 1]
                    List[j + 1] = tmp
    return List
...
def Priority_Adj(List, burst_time):
...
def Priority_Non_Preemptive(Processes, Proc_Num, X, Y):
    global DECIMAL
    DECIMAL = X
    STEP = Y
    # sort processes according to priority and arrival time
    Processes = Priority_Sort(Processes)
    ...
    for i in range(1, Proc_Num):
        Processes[i:] = Priority_Adj(Processes[i:], b_ref)
    ...
    for i in range(0, Proc_Num):
        P_Q.append(
            dict(Task=Processes[i]['Task'],
                 Time_Slots=[(round(float(Processes[i]['Start']), DECIMAL),
                              round(float(Processes[i]['Burst_Time']), DECIMAL))]))
...
def Priority_Preemptive(Processes, Proc_Num, X, Y):
    global DECIMAL
    DECIMAL = X
    STEP = Y
    # arrange processes according to arrival time and if equal arrival time then arranged according to burst time
    ...
    while len(Processes) > 0:
        Processes = Priority_Adj(Processes, round(timer, DECIMAL))
    ...
    for i in range(0, len(Q)):
        ...
        if Indx == -1:
            P_Q.append(dict(Task=Q[i]['Task'], Time_Slots=[
